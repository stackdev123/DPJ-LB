...
import { formatCurrency, formatDate, downloadAsImage, downloadAsExcel } from '../utils';
import { Search, ChevronRight, Truck, Wallet, Plus, Save, ArrowLeft, Printer, ImageDown, Filter, FileText, Download } from 'lucide-react';
...
const SupplierLedger: React.FC<SupplierLedgerProps> = ({ purchases, payments, sales, onPaymentSaved, user }) => {
    ...
    const handleDownloadExcel = () => {
        const dataForExcel = supplierSummary.map(s => ({
            'Nama Supplier': s.name,
            'Beli (Periode)': s.periodPurchase,
            'Bayar (Periode)': s.periodPaid,
            'Total Beli (All Time)': s.totalPurchase,
            'Total Bayar (All Time)': s.totalPaid,
            'Sisa Hutang': s.totalPurchase - s.totalPaid
        }));
        downloadAsExcel(dataForExcel, 'Rekap_Hutang_Supplier');
    };
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 print:hidden">
                ...
                <div className="flex gap-2">
                    <button onClick={handleDownloadExcel} className="flex items-center gap-2 px-3 py-2 bg-green-600 text-white rounded text-sm font-medium hover:bg-green-700 shadow-sm">
