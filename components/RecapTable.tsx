...
const RecapTable: React.FC<RecapTableProps> = ({ purchases, sales, onEditPurchase, onInputSale, onEditSale }) => {
    ...
    const handleDownloadExcel = () => {
        ...
        combinedRows.forEach(row => {
            dataForExcel.push({
                'Tgl Beli': formatDate(row.purchase.date),
                'Supplier': row.purchase.supplier,
                'Kandang': row.purchase.coop,
                'Plat': row.purchase.plate,
                'Modal Beli': row.purchase.totalBuyCost,
                'Customer(s)': row.customers,
                'Beli (Kg)': row.purchase.kg,
                'Jual (Kg)': row.soldKg,
                'Mati (Kg)': row.mortalityKg,
                'Revenue': row.totalRevenue,
                'Ops & Truck': row.totalExpenses,
                'Susut (Kg)': row.shrinkageKg,
                'Laba/Rugi': row.netProfit,
                'Status': row.isFinished ? 'Finished' : 'Open'
            });
        });

        orphanedData.sales.forEach(sale => {
            dataForExcel.push({
                'Tgl Beli': formatDate(sale.date) + ' (Tanpa Induk)',
                'Supplier': '-',
                'Kandang': '-',
                'Plat': '-',
                'Modal Beli': 0,
                'Customer(s)': sale.customerName,
                'Beli (Kg)': 0,
                'Jual (Kg)': sale.soldKg,
                'Mati (Kg)': sale.mortalityKg,
                'Revenue': sale.sellPrice * (sale.soldKg - sale.mortalityKg),
                'Ops & Truck': (sale.unloadingCost || 0) + (sale.operationalCost || 0) + (sale.truckCost || 0),
                'Susut (Kg)': 0,
                'Laba/Rugi': (sale.sellPrice * (sale.soldKg - sale.mortalityKg)) - ((sale.unloadingCost || 0) + (sale.operationalCost || 0) + (sale.truckCost || 0)),
                'Status': 'Orphaned'
            });
        });

        downloadAsExcel(dataForExcel, 'Rekap_Profit_Loss');
    };
        ...
        <div className="space-y-4">
            <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 print:hidden">
                ...
                <div className="flex flex-wrap items-center gap-2">
                    <button onClick={handleDownloadExcel} className="flex items-center gap-1 px-3 py-1.5 bg-green-600 text-white rounded hover:bg-green-700 text-xs font-bold"><Download className="w-3.5 h-3.5" /> Save XLSX</button>
