...
import { formatDate, formatCurrency, downloadAsExcel } from '../utils';
import {
    Loader2, Printer, Edit, FileWarning, Trash2,
    Search, Plus, Filter, ChevronLeft, ChevronRight,
    X, Calendar, AlertCircle, Download
} from 'lucide-react';
...
const SalesList: React.FC<SalesListProps> = ({
    ...
    const handleDownloadExcel = () => {
        const dataForExcel = richSalesData.map(item => ({
            'Tanggal': formatDate(item.date),
            'Customer': item.customerName,
            'Transport (Plat/Kandang/Driver)': `${item.plate} / ${item.coop} / ${item.driver}`,
            'Volume Net (Kg)': (item.soldKg || 0) - (item.mortalityKg || 0),
            'Volume (Ekor)': item.soldHeads,
            'Mati (Kg)': item.mortalityKg,
            'Total Ops': item.totalExtras,
            'Total Tagihan': item.totalInvoice
        }));
        downloadAsExcel(dataForExcel, 'Daftar_Penjualan');
    };
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <div className="flex gap-2 w-full md:w-auto">
                    <button
                        ...
                        className="w-full md:w-auto bg-green-600 hover:bg-green-700 text-white px-5 py-2 rounded-lg text-sm font-bold flex items-center justify-center gap-2 shadow-sm transition-transform active:scale-95"
                    >
