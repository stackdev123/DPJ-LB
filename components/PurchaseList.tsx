...
import { Edit, Trash2, Search, Plus, Truck, Warehouse, Filter, ChevronLeft, ChevronRight, X, Calendar, ShoppingCart, Download } from 'lucide-react';
...
const PurchaseList: React.FC<PurchaseListProps> = ({ purchases, sales, onEditPurchase, onNewPurchase, onSellPurchase, onDeletePurchase, user }) => {
    ...
    const handleDownloadExcel = () => {
        const dataForExcel = filteredPurchases.map(p => ({
            ...
            'Asal (Supplier/Kandang)': `${p.supplier || '-'} / ${p.coop || '-'}`,
            'Transport (Plat/Driver)': `${p.plate || '-'} / ${p.driver || '-'}`,
            'Volume (Kg)': p.kg,
            'Volume (Ekor)': p.heads,
            'Rata-rata': (p.avgWeight || 0).toFixed(2),
            'Harga Beli': p.buyPrice,
            'Total Tagihan': p.totalBuyCost,
        }));
        downloadAsExcel(dataForExcel, 'Daftar_Pembelian');
    };
        ...
        <div className="space-y-4">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <div className="flex gap-2">
                    <button onClick={handleDownloadExcel} className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 shadow-sm whitespace-nowrap">
