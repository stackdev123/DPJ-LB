...
const App: React.FC = () => {
    ...
    const refreshData = async (isAutoSync = false, tablesToRefresh: string[] = []) => {
        ...
        try {
            ...
            if (fetchAll || tablesToRefresh.includes('avt_purchases')) {
                setPurchases(await Storage.getPurchases());
            }
            if (fetchAll || tablesToRefresh.includes('avt_sales')) {
                setSales(await Storage.getSales());
            }
            if (fetchAll || tablesToRefresh.includes('avt_customer_payments')) {
                setCustomerPayments(await Storage.getCustomerPayments());
            }
            if (fetchAll || tablesToRefresh.includes('avt_supplier_payments')) {
                setSupplierPayments(await Storage.getSupplierPayments());
            }


        } catch (e) {
            console.error("Failed to load data", e);
            alert(`Gagal mengambil data dari server. Data lama akan tetap ditampilkan.\nError: ${((e as Error).message) || 'Unknown error'}`);
        } finally {
            setIsLoading(false);
    ...
    return (
      ...
      `}>
                ...
                <div className={`mx-auto print:max-w-none w-full transition-all duration-300 ${[AppView.LEDGER, AppView.SUPPLIER_LEDGER, AppView.RECAP].includes(currentView)
                    ...
                    {currentView === AppView.PURCHASE_LIST && <PurchaseList purchases={purchases} sales={sales} onEditPurchase={handleOpenPurchaseEditModal} onNewPurchase={() => setCurrentView(AppView.PURCHASE)} onDeletePurchase={handleDeletePurchase} onSellPurchase={handleInputSaleFromRecap} user={user} />}
                    ...
                    {currentView === AppView.LEDGER && <LedgerTable data={ledgerData} payments={customerPayments} onViewPayment={handleOpenPaymentModal} onPrintInvoice={(row) => setInvoiceData(row)} onPrintBulkInvoices={(rows) => setBulkInvoiceData(rows)} onShowCustomerStatement={handleShowStatement} onEditSale={handleOpenEditModal} isLoading={isLoading} />}
